...
from numpy import linspace, cosh, exp, sqrt, real, imag, abs, tanh
...
A=1; c=0; x1=0; x2=0.4*R/2                    # Amplitude, vel. & position

u1=1*tanh(A*(x-x1));   # initial condition (IC)
u2=1*tanh(A*(x-x2));   # initial condition (IC)
u0=A*u1;
...
maxtime=2; dt=0.001;               # final time and time step
...
t=0;
utheo=A*tanh(A*(x-c*t-x1))*exp(1j*(A**2)*t);
...
for k in range(1, maxstep + 1):
    ...
    if round(k / stopdisp) == k / stopdisp:
        utheo = A * tanh(A * ((x - c * t - x1 - L) % (R - L) + L)) * exp(-1j * (A**2) * t)
