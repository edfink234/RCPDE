...
namespace InvertedPendulum
{
    ...
    float m = 1.0f;        // Mass
    float Omega = 1.0f;    // Frequency of the harmonic trap
    float A = 1.0f;        // Amplitude of the Gaussian potential
    float sigma = 1.0f;    // Width of the Gaussian potential
    float T = 10.0f;       // Final time
    ...
    auto sech = [](float x){return 1.0f/cosh(x);};
...
//        return V_MT + V_G;
        float V_MT = 0.5 * Omega * Omega * x * x;
        float temp = sech(A*(x - xi));
        float V_SECH = A*A*temp*temp;
        return V_MT + V_SECH;
    }
...
//        return -(Omega * Omega * (x - xi)) - (2.0 * A * (x - xi) / sigma) * exp(-(x - xi) * (x - xi) / sigma);
        float temp = A * (x - xi);
        float temp1 = sech(temp);
        return -(Omega * Omega * x) + (2 * A*A*A * temp1*temp1 * tanh(temp));
    }
...
struct Board
{
    ...
    float loss_func()
    {
        ...
        float diff_xi_T = (x_star - expression_evaluator(this->params, this->pieces, T));
...
//        float delta_xi/*, delta_t*/;
        Eigen::VectorXf xi_val = this->expression_evaluator(params, this->pieces);
...
////            if (delta_t > 0)
//            {
//                smoothness_penalty += (xi_deriv.size() - i) * ((delta_xi*delta_xi)/*/(delta_t*delta_t)*/);
//            }
//        }
...
//        }
    ...
    float loss_func(const Eigen::VectorXf& params)
    {
        ...
        float diff_xi_T = (x_star - expression_evaluator(params, this->pieces, T));
        this->MSE_curr += diff_xi_T*diff_xi_T;
        float diff_xi_0 = expression_evaluator(params, this->pieces, 0.0f);
        this->MSE_curr += diff_xi_0*diff_xi_0;
        //Take the first derivative of xi wrt to t
...
////            if (delta_t > 0)
//            {
//                smoothness_penalty += delta_xi;//(xi_deriv.size() - i) * ((delta_xi*delta_xi)/*/(delta_t*delta_t)*/);
//            }
//        }
//        this->MSE_curr += 0.005f*smoothness_penalty;
        if (Variance(xi_val) > this->upperVar)
...
//                    stack.push_back(((expression_type == "postfix") ? (right_operand.array().pow(left_operand.array())) : (left_operand.array().pow(right_operand.array()))));
    ...
    float operator()(Eigen::VectorXf& x, Eigen::VectorXf& grad)
    {
        ...
        else if (this->fit_method == "LevenbergMarquardt")
        {
            using namespace InvertedPendulum;
            State state;
            state.x = 1.0f;  // Initial position x0
            state.v = 0.0f;  // Initial velocity v0
            ...
            grad(0) = x_star - state.x;

            // fvec(1): Velocity threshold condition
            float abs_v = std::abs(state.v);
            if (abs_v > v_th)
            {
                grad(1) = abs_v - v_th;
            ...
            {
                grad(1) = 0.0f;  // No penalty if velocity is within the threshold
            }
            
            // fvec(2): Final condition on xi(T)
            float diff_xi_T = x_star - expression_evaluator(x, this->pieces, T);
            grad(2) = diff_xi_T;
