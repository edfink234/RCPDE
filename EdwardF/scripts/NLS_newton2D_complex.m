function resid = nls2d_msd(psi, params)
    ...
    term_tl = (d2X(2, 2) * X(2, 2) + d2Y(2, 2) * Y(2, 2)) / dens(2, 2);
    term_tr = (d2X(2, end-1) * X(2, end-1) + d2Y(2, end-1) * Y(2, end-1)) / dens(2, end-1);
    term_bl = (d2X(end-1, 2) * X(end-1, 2) + d2Y(end-1, 2) * Y(end-1, 2)) / dens(end-1, 2);
    term_br = (d2X(end-1, end-1) * X(end-1, end-1) + d2Y(end-1, end-1) * Y(end-1, end-1)) / dens(end-1, end-1);

    Omega_tl = term_tl - 2 * (dens(2, 2) - dens(1, 1) + V(2, 2) - V(1, 1));
    Omega_tr = term_tr - 2 * (dens(2, end-1) - dens(1, end) + V(2, end-1) - V(1, end));
    Omega_bl = term_bl - 2 * (dens(end-1, 2) - dens(end, 1) + V(end-1, 2) - V(end, 1));
    Omega_br = term_br - 2 * (dens(end-1, end-1) - dens(end, end) + V(end-1, end-1) - V(end, end));

    Xdd_tl = Omega_tl * X(1, 1);
    Xdd_tr = Omega_tr * X(end, 1);
    Xdd_bl = Omega_bl * X(1, end);
    Xdd_br = Omega_br * X(end, end);

    Ydd_tl = Omega_tl * Y(1, 1);
    Ydd_tr = Omega_tr * Y(end, 1);
    Ydd_bl = Omega_bl * Y(1, end);
    Ydd_br = Omega_br * Y(end, end);
...
params.mu = 1; % Temporal frequency = A^2
npts_x = params.npts_x;
npts_y = params.npts_y;
...
w = params.mu;
A = sqrt(w);
x_0 = 0.0;  % Initial guess location
y_0 = 0.0;
